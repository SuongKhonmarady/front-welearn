...
const Analytics = ({ 
  ...
  return (
    <div className="space-y-8">
          ...
          <div className="flex items-center gap-4 text-sm text-gray-600">
            ...
            {aggregatedScholarshipData.length > 0 && (
              <span className="bg-gray-100 px-3 py-1 rounded-full font-medium">
                Total: {aggregatedScholarshipData.reduce((sum, item) => sum + item['Scholarships Added'], 0)} scholarships
              </span>
            )}
          </div>
        </div>
        ...
        {aggregatedScholarshipData.length > 0 ? (
          <div className="space-y-4">
            <ResponsiveContainer width="100%" height={450}>
              <BarChart 
                data={aggregatedScholarshipData}
                margin={{
                  top: 20,
                  right: 30,
                  left: 20,
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                <div className="bg-white p-3 rounded-lg border">
                  ...
                  <p className="font-bold text-blue-600">
                    {Math.max(...aggregatedScholarshipData.map(d => d['Scholarships Added']))}
                  </p>
                </div>
                <div className="bg-white p-3 rounded-lg border">
                  <p className="text-xs text-gray-500">Average</p>
                  <p className="font-bold text-green-600">
                    {(aggregatedScholarshipData.reduce((sum, item) => sum + item['Scholarships Added'], 0) / aggregatedScholarshipData.length).toFixed(1)}
                  </p>
                </div>
                <div className="bg-white p-3 rounded-lg border">
                  <p className="text-xs text-gray-500">Total Days</p>
                  <p className="font-bold text-purple-600">{aggregatedScholarshipData.length}</p>
                </div>
                <div className="bg-white p-3 rounded-lg border">
                  <p className="text-xs text-gray-500">Active Days</p>
                  <p className="font-bold text-orange-600">
                    {aggregatedScholarshipData.filter(d => d['Scholarships Added'] > 0).length}
                  </p>
                </div>
              </div>
            </div>
      ...
      {/* Recent Activity */}
      {timeframe === 'daily' && aggregatedScholarshipData.length > 0 && (
        <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
          <h2 className="text-2xl font-semibold mb-6 text-gray-800">🕐 Recent Daily Activity</h2>
          <div className="space-y-3">
            {aggregatedScholarshipData.slice(-7).reverse().map((day, index) => (
